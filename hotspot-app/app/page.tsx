...
import { getCountryImageUrl, isLocalCountryImage } from "./data/countryImages";
...
function CountryIllustration({
  ...
  const src = getCountryImageUrl(countryId, countryTitle);
  const local = isLocalCountryImage(countryId);
  return (
    <figure className="space-y-2">
      <Image
        src={src}
        alt={`${countryTitle} — ${highlight} highlight`}
        width={1200}
        height={800}
        className="aspect-[3/2] w-full max-w-xl rounded-md object-cover object-center mx-auto shadow-md"
        sizes="(max-width: 1024px) 100vw, 42rem"
      />
      <figcaption className="text-center text-xs leading-snug px-2 text-stone-500">
        {local
          ? `Photograph used for ${countryTitle} only.`
          : `Unique stock image for ${countryTitle} (${highlight} highlight).`}
      </figcaption>
    </figure>
  );
}
    ...
    <div className="min-h-screen p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        <section
          ...
          className="hotspot-card hotspot-card--accent mb-8"
        >
          <h1 tabIndex={-1} className="hotspot-page-title mb-3">
            Interactive page to explore some of Africa&apos;s countries
          </h1>
          <p className="text-center leading-relaxed text-stone-700">
            ...
            dots mark each hotspot and the selected country shows its flag. Each country has its
            own photo in the panel (no image is
            shared between countries). Use <strong>Compare regions</strong> for a side-by-side
            table, <strong>Reset view</strong> to clear filters and refit the map, and{" "}
            <strong>Copy share link</strong> for URLs like{" "}
            <code className="rounded bg-amber-100 px-1.5 py-0.5 text-sm text-amber-950">#region-3</code>{" "}
            (region index in the sorted list) plus optional filters.
          </p>
        </section>

        <section className="hotspot-card mb-8">
          <div className="mb-4 flex items-center justify-between flex-wrap gap-3">
            <h2 className="hotspot-section-title">Data panel updates</h2>
            <div className="hotspot-count-pill">
              Showing {filteredCountries.length} of {countries.length} countries
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className="flex flex-col gap-1">
              <span className="text-sm font-medium text-emerald-900">Search</span>
              <input
                type="text"
                value={searchFilter}
                onChange={(e) => {
            ...
            <label className="flex flex-col gap-1">
              <span className="text-sm font-medium text-emerald-900">Region</span>
              <select
                value={regionFilter}
                onChange={(e) => {
                  const v = e.target.value;
            ...
            <label className="flex flex-col gap-1">
              <span className="text-sm font-medium text-emerald-900">Highlight</span>
              <select
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-orange-200/80 pt-4">
            <button type="button" onClick={handleResetView} className="hotspot-btn-secondary">
              Reset view
            </button>
            <button
              type="button"
              onClick={() => void handleShareDeepLink()}
              className="hotspot-btn-primary"
            >
              {shareCopied ? "Link copied" : "Copy share link"}
            </button>
            <span className="text-xs text-stone-600">
              Deep link uses sorted region index, e.g.{" "}
              <code className="rounded bg-amber-100 px-1 text-amber-950">#region-3</code>
            </span>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch lg:min-h-0">
          <div
            className={[
              "hotspot-card relative z-0 min-w-0 overflow-hidden p-0 lg:h-[80vh]",
          ...
          <div
            ...
            className="hotspot-card relative z-10 hidden min-w-0 overflow-y-auto overflow-x-hidden lg:block lg:h-[80vh]"
          >
              ...
              <div className="space-y-6">
                <div className="pb-4">
                  ...
                  <div className="mb-3 flex flex-wrap items-center gap-2">
                    <span className="hotspot-badge-region">{selectedCountryData.region}</span>
                    <span className="hotspot-badge-highlight">{selectedCountryData.highlight}</span>
                  </div>
      ...
      {isPhoneViewport && isMobileInfoModalOpen && (
        ...
        >
          ...
          <div
            ...
            className="relative z-10 max-h-[80vh] overflow-y-auto overflow-x-hidden rounded-t-2xl border-t-4 border-t-emerald-600 bg-white p-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] shadow-2xl"
          >
            {filteredCountries.length > 0 ? (
              <>
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setIsMobileInfoModalOpen(false)}
                    className="hotspot-btn-secondary px-3 py-1"
                  >
                    Close
                  </button>
                </div>
