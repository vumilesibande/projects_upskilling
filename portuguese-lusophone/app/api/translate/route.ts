import {
  isBlockedTranslation,
  looksLikePortuguese,
  pickBestMemoryMatch,
} from "@/lib/translation-safety";
...
async function fetchMyMemory(
  ...
  const sourceNorm = text.trim().toLowerCase();
  const exactMatch = data.matches
    ?.find((entry) => {
      const segmentNorm = entry.segment?.trim().toLowerCase();
      const translation = entry.translation?.trim();
      return !!translation && segmentNorm === sourceNorm;
    })
    ?.translation?.trim();

  if (exactMatch && isUsableTranslation(exactMatch, text)) {
    return exactMatch;
  }
  ...
  try {
    ...
    if (!pt) {
      return Response.json(
        {
          error:
            "Could not translate this text right now. Try a shorter phrase.",
        },
        { status: 422 },
      );
    }
